...
def get_duplicate_pairs() -> None:
    ...
    schreibt ähnliche Paare (basierend auf Jaccard-Ähnlichkeit) in eine Datei.
    """
    block = generate_blocking()
    union = extend_union_jaccard(block, 'title')
    union.write_pairs_to_file()


def extend_union_jaccard(groups: Iterator[Tuple[tuple, pl.DataFrame]],column: str,uf: Optional[UnionFind] = None) -> UnionFind:
    """
...
def _process_group(args: Tuple[pl.DataFrame, str]) -> list[Tuple[str, str]]:
    ...
    texts = df[column].cast(str).to_list()
